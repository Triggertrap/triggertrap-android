...
public class DrawerFragmentHandler {
    ...
    public void onDrawerSelected(Context context, String tag, boolean playAnimation, boolean hasBeenRotated) {
        if (!mInitialized) {
            return;
        }
        if (!hasBeenRotated) {
...
//        ft.setCustomAnimations(R.animator.fragment_fade_in, R.animator.fragment_slide_out);
            ...
            for (int i = 0; i < mDrawerPanes.size(); i++) {
                ...
                if (drawerPaner.tag.equals(tag)) {
                    mLastDrawerPane = drawerPaner;
                    mLastDrawerPane.fragment = frag;
                    break;
                }
            }
        }
